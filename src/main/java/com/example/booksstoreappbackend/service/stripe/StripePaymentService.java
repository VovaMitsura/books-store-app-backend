package com.example.booksstoreappbackend.service.stripe;

import com.example.booksstoreappbackend.exception.ApplicationExceptionHandler;
import com.example.booksstoreappbackend.exception.PaymentException;
import com.example.booksstoreappbackend.model.CreditCard;
import com.example.booksstoreappbackend.model.Order;
import com.example.booksstoreappbackend.service.PaymentProvider;
...
public class StripePaymentService implements PaymentProvider {
  ...
  private Charge chargeCreditCard(CreditCard card, Order order) {
    ...
    var totalAmountWithDiscount = orderDetails.stream()
            .mapToDouble(details -> details.getTotalPrice() *
                    (details.getBook().getDiscount() != null ?
                            (1 - details.getBook().getDiscount().getPercentage() / 100) : 1))
            .sum();

    totalAmountWithDiscount = totalAmountWithDiscount * 100;

    Map<String, Object> chargeParams = new HashMap<>();
    Map<String, Object> metaParams = new HashMap<>();
    chargeParams.put("amount", (int) totalAmountWithDiscount);
    ...
    try {
      ...
      throw new PaymentException(ApplicationExceptionHandler.PAYMENT_EXCEPTION,
              String.format("Exception occur during paying for order [%s] with error [%s]",
